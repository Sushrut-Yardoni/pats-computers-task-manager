...
export default function TodoManagementSection({
  ...
  const processDrop = async (
    ...
    let updatedRole = dragged.created_by_role || "Admin";
    if (isChangingUser) {
    ...
    try {
      if (isChangingUser) {
        const historyEntry = {
          ...
          edited_by: `${currentUser?.name || "Admin"} (${currentUser?.role || "Admin"})`,
          before: {
            title: dragged.title,
      ...
      <div 
        ...
        className={`bg-white border rounded-xl shadow-3xs hover:shadow-md hover:translate-y-[-1px] transition-all duration-200 flex flex-col shrink-0 overflow-hidden cursor-pointer relative group ${
          isBeingDragged 
            ...
            : isTargetOfDrag 
            ? (dropPosition === "above" 
                ? "border-t-4 border-t-indigo-500 ring-2 ring-indigo-200 shadow-lg scale-[1.01]" 
                : "border-b-4 border-b-indigo-500 ring-2 ring-indigo-200 shadow-lg scale-[1.01]")
            : activeTab === "deleted" 
        ...
        <div className="p-3.5 flex-grow flex flex-col justify-between space-y-3">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1.5">
              {isDraggable && (
                <span title="Drag up/down to reorder or drag to another user column to reassign" className="text-slate-300 group-hover:text-indigo-600 transition-colors cursor-grab active:cursor-grabbing p-0.5">
            ...
            {viewMode === "userColumns" && (
              <div className="relative shrink-0">
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-2 bg-slate-50 hover:bg-slate-100 border border-slate-200 focus:border-blue-500 rounded-xl text-xs font-semibold text-slate-700 transition-all cursor-pointer shadow-3xs"
        ...
        {isLoading ? (
          ...
          getUserGroups().length === 0 ? (
            <div className="bg-slate-50/80 border-2 border-dashed border-slate-200 rounded-3xl p-8 text-center space-y-3">
              <div className="p-3 bg-indigo-50 text-indigo-600 rounded-2xl w-max mx-auto">
                    ...
                    <div 
                      ...
                      className={`w-72 sm:w-80 shrink-0 rounded-2xl p-3.5 flex flex-col h-[500px] shadow-xs transition-all relative ${
                        isBeingColumnDragged
                          ...
                          : isTargetColumn
                            ? (columnDropSide === "left"
                                ? "border-l-4 border-l-indigo-500 border-y border-r border-slate-300 ring-4 ring-indigo-100 scale-[1.01] shadow-md"
                                : "border-r-4 border-r-indigo-500 border-y border-l border-slate-300 ring-4 ring-indigo-100 scale-[1.01] shadow-md")
                            : isColumnDragOver && !draggedColumnUser
                              ? "bg-indigo-100/70 border-2 border-indigo-500 ring-4 ring-indigo-200/80 scale-[1.01]"
                              : isSelf 
                    ...
                    >
                      ...
                      >
                        <div className="flex items-center gap-1.5 min-w-0">
                          <GripVertical className="h-4 w-4 text-slate-300 group-hover/header:text-indigo-600 shrink-0 transition-colors" />
                        ...
                        <div className="flex items-center gap-1.5 shrink-0">
                          <span className="bg-indigo-600 text-white font-mono text-[10px] font-extrabold px-2 py-0.5 rounded-full">
                            {group.todos.length}
                          </span>
                          <button
                            type="button"
